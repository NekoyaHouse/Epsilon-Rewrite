...
public class ContentPanel implements IComponent {
	...
	private void renderSearchBox(RendererSet set, float x, float y, float w, float h, float guiScale, boolean focused,
			boolean hovered, String text, float alpha) {
		Color bgColor = focused ? applyAlpha(new Color(50, 50, 50, 200), alpha)
				: (hovered ? applyAlpha(new Color(40, 40, 40, 200), alpha)
						: applyAlpha(new Color(30, 30, 30, 200), alpha));
		...
		if (focused && (System.currentTimeMillis() % 1000 > 500))
			display += "_";
		set.font().addText(display, x + 6 * guiScale, y + h / 2 - 7 * guiScale, guiScale * 0.9f,
				text.isEmpty() && !focused ? applyAlpha(Color.GRAY, alpha) : applyAlpha(Color.WHITE, alpha));
	}

	private void renderIconBox(RendererSet set, float x, float y, float w, float h, float guiScale, boolean hovered,
			float alpha) {
		set.bottomRoundRect().addRoundRect(x, y, w, h, 8f * guiScale,
				hovered ? applyAlpha(new Color(40, 40, 40, 200), alpha)
						: applyAlpha(new Color(30, 30, 30, 200), alpha));
		...
		set.font().addText("<", iconX, iconY - 1, iconScale, applyAlpha(new Color(200, 200, 200), alpha),
				StaticFontLoader.ICONS);
	}

	private void renderScrollbar(RendererSet set, float x, float y, float w, float h, float thumbY, float thumbH,
			boolean dragging, boolean hovered, boolean thumbHovered, float alpha) {
		Color trackColor = hovered ? applyAlpha(new Color(255, 255, 255, 28), alpha)
				: applyAlpha(new Color(255, 255, 255, 18), alpha);
		Color thumbColor = dragging ? applyAlpha(new Color(255, 255, 255, 90), alpha)
				: (thumbHovered ? applyAlpha(new Color(255, 255, 255, 75), alpha)
						: applyAlpha(new Color(255, 255, 255, 55), alpha));
	...
	private void setupScissor(float areaX, float areaY, float areaW, float areaH, float pxScale, int fbW, int fbH,
			int guiH) {
	...
	private float handleScrollDrag(float scrollTarget, float maxScroll, float thumbH, float scrollbarH, float mouseY,
			float dragStartMouseY, float dragStartScroll) {
	...
	private float handleScrollClick(float scrollTarget, float maxScroll, float thumbH, float thumbY, float scrollbarX,
			float scrollbarY, float scrollbarW, float scrollbarH, float mouseX, float mouseY) {
		float thumbTravel = Math.max(0.0f, scrollbarH - thumbH);
		if (thumbTravel <= 0.0f)
			return scrollTarget;
		if (MouseUtils.isHovering(scrollbarX, thumbY, scrollbarW, thumbH, mouseX, mouseY))
			return scrollTarget;
		float ratio = Mth.clamp((mouseY - scrollbarY - thumbH / 2.0f) / thumbTravel, 0.0f, 1.0f);
		return ratio * maxScroll;
	}

	private float handleMouseScroll(float scrollTarget, float maxScroll, float areaX, float areaY, float areaW,
			float areaH, float scrollbarW, double mouseX, double mouseY, double scrollY) {
		if (maxScroll <= 0.0f)
			return scrollTarget;
		if (!MouseUtils.isHovering(areaX, areaY, areaW + scrollbarW, areaH, mouseX, mouseY))
			return scrollTarget;
	...
	private void renderListView(RendererSet set, int mouseX, int mouseY, float deltaTicks, float alpha) {
		...
		renderIconBox(set, lastIconBoxX, lastIconBoxY, iconBoxWidth, searchHeight, guiScale,
				MouseUtils.isHovering(lastIconBoxX, lastIconBoxY, iconBoxWidth, searchHeight, mouseX, mouseY), alpha);
		...
		renderSearchBox(set, lastSearchBoxX, lastSearchBoxY, searchBoxWidth, searchHeight, guiScale, listSearchFocused,
				MouseUtils.isHovering(lastSearchBoxX, lastSearchBoxY, searchBoxWidth, searchHeight, mouseX, mouseY),
				listSearchText, alpha);
		...
		for (ModuleCard card : moduleCards) {
			boolean matchesSearch = listSearchText.isEmpty()
					|| card.module.getTranslatedName().toLowerCase().startsWith(listSearchText.toLowerCase());
		...
		if (listDraggingScrollbar && listMaxScroll > 0.0f) {
			listScrollTarget = handleScrollDrag(listScrollTarget, listMaxScroll, lastThumbH, lastScrollbarH, mouseY,
					listScrollbarDragStartMouseY, listScrollbarDragStartScroll);
		}

		float pxScale = (float) mc.getWindow().getGuiScale();
		int scX = Mth.clamp(Mth.floor(lastListX * pxScale), 0, mc.getWindow().getWidth());
		int scY = Mth.clamp(Mth.floor((mc.getWindow().getGuiScaledHeight() - (lastListY + lastListH)) * pxScale), 0,
		...
		if (listMaxScroll > 0.0f) {
			renderScrollbar(set, lastScrollbarX, lastListY, lastScrollbarW, lastListH, lastThumbY, lastThumbH,
					listDraggingScrollbar,
					MouseUtils.isHovering(lastScrollbarX, lastListY, lastScrollbarW, lastListH, mouseX, mouseY),
					MouseUtils.isHovering(lastScrollbarX, lastThumbY, lastScrollbarW, lastThumbH, mouseX, mouseY),
					alpha);
		}
	}
	...
	public void renderGui(DeltaTracker partialTick) {
		...
		setupScissor(lastSettingsX, lastSettingsY, lastSettingsW, lastSettingsH, (float) mc.getWindow().getGuiScale(),
				mc.getWindow().getWidth(), mc.getWindow().getHeight(), mc.getWindow().getGuiScaledHeight());
	...
	private boolean listViewMouseClicked(MouseButtonEvent event, boolean focused) {
		...
		if (MouseUtils.isHovering(lastSearchBoxX, lastSearchBoxY, lastSearchBoxW, lastSearchBoxH, event.x(),
				event.y())) {
			if (event.button() == 1) {
				listSearchText = "";
				listScrollTarget = 0.0f;
		...
		if (event.button() == 0 && listMaxScroll > 0.0f && MouseUtils.isHovering(lastScrollbarX, lastScrollbarY,
				lastScrollbarW, lastScrollbarH, event.x(), event.y())) {
			...
			listScrollTarget = handleScrollClick(listScrollTarget, listMaxScroll, lastThumbH, lastThumbY,
					lastScrollbarX, lastScrollbarY, lastScrollbarW, lastScrollbarH, (float) event.x(),
					(float) event.y());
			listDraggingScrollbar = true;
			listScrollbarDragStartMouseY = (float) event.y();
			listScrollbarDragStartScroll = listScrollTarget;
		...
		if (event.button() == 0) {
			for (ModuleCard card : moduleCards) {
				if (card.width > 0 && card.height > 0
						&& MouseUtils.isHovering(card.x, card.y, card.width, card.height, event.x(), event.y())) {
					card.module.toggle();
					return true;
				}
			}
	...
	private boolean listViewMouseScrolled(double mouseX, double mouseY, double scrollY) {
		listScrollTarget = handleMouseScroll(listScrollTarget, listMaxScroll, lastListX, lastListY, lastListW,
				lastListH, lastScrollbarW, mouseX, mouseY, scrollY);
		return listMaxScroll > 0.0f
				&& MouseUtils.isHovering(lastListX, lastListY, lastListW + lastScrollbarW, lastListH, mouseX, mouseY);
	}
	...
	private void renderSettingsView(RendererSet set, int mouseX, int mouseY, float deltaTicks, float alpha) {
		...
		if (!settingsComponent.isExiting()) {
			...
			lastIconBoxH = searchHeight;
			renderIconBox(set, lastIconBoxX, lastIconBoxY, lastIconBoxW, searchHeight, guiScale,
					MouseUtils.isHovering(lastIconBoxX, lastIconBoxY, lastIconBoxW, searchHeight, mouseX, mouseY),
					alpha);
			...
			renderSearchBox(set, lastSettingsSearchBoxX, lastSettingsSearchBoxY, lastSettingsSearchBoxW, searchHeight,
					guiScale, settingsSearchFocused, MouseUtils.isHovering(lastSettingsSearchBoxX,
							lastSettingsSearchBoxY, lastSettingsSearchBoxW, searchHeight, mouseX, mouseY),
					settingsSearchText, alpha);
		}
		...
		float contentH = 8.0f * guiScale + titleH + 6.0f * guiScale
				+ (itemCount > 0 ? itemCount * 18.0f * guiScale + Math.max(0, itemCount - 1) * 4.0f * guiScale : 0)
				+ 8.0f * guiScale;
		...
		lastSettingsThumbH = settingsMaxScroll <= 0.0f ? lastSettingsH
				: Math.max(12.0f * guiScale, lastSettingsH * (lastSettingsH / contentH));
		float thumbTravel = Math.max(0.0f, lastSettingsH - lastSettingsThumbH);
		lastSettingsThumbY = settingsMaxScroll <= 0.0f ? lastSettingsY
				: lastSettingsY + (settingsScrollOffset / settingsMaxScroll) * thumbTravel;

		if (settingsDraggingScrollbar && settingsMaxScroll > 0.0f) {
			settingsScrollTarget = handleScrollDrag(settingsScrollTarget, settingsMaxScroll, lastSettingsThumbH,
					lastSettingsScrollbarH, mouseY, settingsScrollbarDragStartMouseY, settingsScrollbarDragStartScroll);
		}

		

		RendererSet settingsSet = new RendererSet(settingsRoundRect, set.topRoundRect(), set.texture(), settingsFont,
				pickingRound, pickingRect, pickerRound, pickingText);
		if (!settingsComponent.isExiting()) {
		...
		if (settingsMaxScroll > 0.0f && !settingsComponent.isExiting()) {
			renderScrollbar(set, lastSettingsScrollbarX, lastSettingsY, lastSettingsScrollbarW, lastSettingsH,
					lastSettingsThumbY, lastSettingsThumbH, settingsDraggingScrollbar,
					MouseUtils.isHovering(lastSettingsScrollbarX, lastSettingsY, lastSettingsScrollbarW, lastSettingsH,
							mouseX, mouseY),
					MouseUtils.isHovering(lastSettingsScrollbarX, lastSettingsThumbY, lastSettingsScrollbarW,
							lastSettingsThumbH, mouseX, mouseY),
					alpha);
		}
	}
	...
	private boolean settingsViewMouseClicked(MouseButtonEvent event, boolean focused) {
		...
		if (ColorSettingComponent.hasActivePicker()
				&& !ColorSettingComponent.isMouseOutOfPicker((int) event.x(), (int) event.y())) {
		...
		if (event.button() == 0 && MouseUtils.isHovering(lastIconBoxX, lastIconBoxY, lastIconBoxW, lastIconBoxH,
				event.x(), event.y())) {
			settingsExitRequested = true;
			return true;
		}

		if (MouseUtils.isHovering(lastSettingsSearchBoxX, lastSettingsSearchBoxY, lastSettingsSearchBoxW,
				lastSettingsSearchBoxH, event.x(), event.y())) {
			if (event.button() == 1) {
				settingsSearchText = "";
				settingsScrollTarget = 0.0f;
			}
		...
		if (event.button() == 0 && settingsMaxScroll > 0.0f && MouseUtils.isHovering(lastSettingsScrollbarX,
				lastSettingsScrollbarY, lastSettingsScrollbarW, lastSettingsScrollbarH, event.x(), event.y())) {
			if (MouseUtils.isHovering(lastSettingsScrollbarX, lastSettingsThumbY, lastSettingsScrollbarW,
					lastSettingsThumbH, event.x(), event.y())) {
			...
			settingsScrollTarget = handleScrollClick(settingsScrollTarget, settingsMaxScroll, lastSettingsThumbH,
					lastSettingsThumbY, lastSettingsScrollbarX, lastSettingsScrollbarY, lastSettingsScrollbarW,
					lastSettingsScrollbarH, (float) event.x(), (float) event.y());
			settingsDraggingScrollbar = true;
			settingsScrollbarDragStartMouseY = (float) event.y();
			settingsScrollbarDragStartScroll = settingsScrollTarget;
			return true;
	...
	private boolean settingsViewMouseReleased(MouseButtonEvent event) {
		...
		float guiScale = ClickGui.INSTANCE.scale.getValue().floatValue();
		return MouseUtils.isHovering(x, y, this.width * guiScale, this.height * guiScale, event.x(), event.y())
				&& settingsComponent.mouseReleased(event);
	}

	private boolean settingsViewMouseScrolled(double mouseX, double mouseY, double scrollY) {
		if (settingsComponent == null || settingsMaxScroll <= 0.0f)
			return false;
		settingsScrollTarget = handleMouseScroll(settingsScrollTarget, settingsMaxScroll, lastSettingsX, lastSettingsY,
				lastSettingsW, lastSettingsH, lastSettingsScrollbarW, mouseX, mouseY, scrollY);
		return MouseUtils.isHovering(lastSettingsX, lastSettingsY, lastSettingsW + lastSettingsScrollbarW,
				lastSettingsH, mouseX, mouseY);
	}
	...
	private static final class ModuleCard {
		...
		private void render(RoundRectRenderer round, TextRenderer text, int mouseX, int mouseY, float guiScale,
			...
			int a = Mth.clamp(
					(int) (offColor.getAlpha() + (onColor.getAlpha() - offColor.getAlpha()) * et) + (int) (24.0f * ht),
					0, 255);
			...
			if (descWidth > maxDescWidth && descWidth > 0)
				descScale *= maxDescWidth / descWidth;
			...
			text.addText(module.getTranslatedName(), renderX + (rw - (Math.min(nameWidth, maxNameWidth))) / 2f,
					startY - 0.6f * guiScale, nameScale, new Color(255, 255, 255, textAlpha));
			text.addText(module.getDescription(), renderX + (rw - (Math.min(descWidth, maxDescWidth))) / 2f,
					startY + nameHeight + 3 * guiScale - 0.2f * guiScale, descScale,
					new Color(200, 200, 200, textAlpha));
		}
	}
	...
	public void render(RendererSet set, int mouseX, int mouseY, float deltaTicks, float alpha) {
		...
		if (ClickGui.INSTANCE.isSidebarBlur()) {
			BlurShader.INSTANCE.drawBlur(x, y, this.width * guiScale, this.height * guiScale, 0, radius, radius, 0,
					ClickGui.INSTANCE.getBlurStrength());
		}

		set.bottomRoundRect().addRoundRect(x, y, this.width * guiScale, this.height * guiScale, 0, radius, radius, 0,
				new Color(0, 0, 0, 25));
		...
		if (currentState == 2) {
			...
			if (viewAnimation.getValue() >= 0.99f)
				currentState = 1;

			setupScissor(this.x, this.y, this.width * guiScale, this.height * guiScale,
					(float) mc.getWindow().getGuiScale(), mc.getWindow().getWidth(), mc.getWindow().getHeight(),
		...
		if (currentState == 0) {
			...
			if (open != null) {
				...
				if (settingsComponent != null) {
					...
					float targetW = Math.max(0.0f,
							this.width * guiScale - padding * 2 - 4.0f * guiScale - 4.0f * guiScale);
					settingsComponent.setFilterText("");
					int itemCount = settingsComponent.getFilteredVisibleCount();
					float titleH = 18.0f * guiScale;
					float targetH = 8.0f * guiScale + titleH + 6.0f * guiScale
							+ (itemCount > 0
									? itemCount * 18.0f * guiScale + Math.max(0, itemCount - 1) * 4.0f * guiScale
									: 0)
							+ 8.0f * guiScale;
					settingsComponent.initAnimation(sourceCardX, sourceCardY, sourceCardW, sourceCardH, targetX,
